import { describe, expect, it } from "vitest";

import {
  buildJpegPdf,
  dataUrlToUint8Array,
  defaultCanvasPdfFilename,
} from "./canvas-pdf-export";

/** Minimal valid JPEG (1×1). */
const TINY_JPEG = Uint8Array.from([
  ,
]);

describe("buildJpegPdf", () => {
  it("produces a PDF header and embeds the JPEG", () => {
    const pdf = buildJpegPdf(TINY_JPEG, 800, 600);
    const text = new TextDecoder().decode(pdf.slice(0, 32));
    expect(text.startsWith("%PDF-1.4")).toBe(true);
    expect(new TextDecoder().decode(pdf).includes("/MediaBox [0 0 800 600]")).toBe(true);
    expect(new TextDecoder().decode(pdf).includes("%%EOF")).toBe(true);

    let found = false;
    for (let i = 0; i < pdf.length - 1; i += 1) {
      if (pdf[i] === 0xff && pdf[i + 1] === 0xd8) {
        found = true;
        break;
      }
    }
    expect(found).toBe(true);
  });
});

describe("dataUrlToUint8Array", () => {
  it("decodes base64 data URLs", () => {
    const bytes = dataUrlToUint8Array("data:image/jpeg;base64,/9j/");
    expect(bytes[0]).toBe(0xff);
    expect(bytes[1]).toBe(0xd8);
  });
});

describe("defaultCanvasPdfFilename", () => {
  it("uses e2-canvas prefix and timestamp", () => {
    const name = defaultCanvasPdfFilename(new Date("2026-08-17T09:05:00"));
    expect(name).toBe("e2-canvas-20260817-0905.pdf");
  });
});
